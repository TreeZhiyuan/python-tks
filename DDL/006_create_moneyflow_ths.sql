...
CREATE TABLE IF NOT EXISTS `moneyflow_ths` (
    ...
    `net_amount` DECIMAL(20,4) DEFAULT NULL COMMENT '资金净流入(万元)',
    `net_d5_amount` DECIMAL(20,4) DEFAULT NULL COMMENT '5日主力净额(万元)',
    ...
    KEY `idx_moneyflow_ths_ts_code` (`ts_code`),
    KEY `idx_moneyflow_ths_name` (`name`)
) ENGINE=InnoDB
